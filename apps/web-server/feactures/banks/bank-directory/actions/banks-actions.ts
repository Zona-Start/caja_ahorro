...
export const getBanksAction = async () => {
  ...
  if (error) {
    ...
    throw new Error(error.message || 'Error fetching banks data');
  }
  return data;
};
...
export const getPaginatedBanksAction = async (params: {
  ...
  const [error, response] = await safeFetchApi(
    banksPaginationResponseSchema,
    `/banks-directory/paginated?${searchParams}`,
    'GET',
  );

  if (error) {
    console.error('Error:', error);
    throw new Error(error.message || 'Error fetching banks directory data');
  }
...
export const createBankAction = async (payload: Banks) => {
  ...
  if (error) {
    console.error('Error:', error);
    throw new Error(error.message || 'Error create bank data');
  }
...
export const updateBankAction = async (payload: Banks) => {
  ...
  if (error) {
    console.error('Error:', error);
    throw new Error(error.message || 'Error update bank data');
  }
...
export const deleteBankAction = async (id: number) => {
  ...
  if (error) {
    console.error('Error:', error);
    throw new Error(error.message || 'Error delete bank data');
  }
...
export const getBankByIdAction = async (id: number) => {
  ...
  if (error) {
    console.error('Error:', error);
    throw new Error(error.message || 'Error fetching bank data');
  }
