...
import {
  ESTATUS_TYPES,
  PAYROLL_TYPE,
} from '../../schemas/ordinary-loans-options';
...
export function useAssociatesTableFilters() {
  ...
  const [payrollFilter, setPayrollFilter] = useQueryState(
    'payroll',
    searchParams.q.withOptions({ shallow: false }).withDefault(''),
  );
  ...
  const resetFilters = useCallback(() => {
    ...
    setPayrollFilter(null);
    setPage(1);
  }, [setSearchQuery, setStatusFilter, setPayrollFilter, setPage]);

  const isAnyFilterActive = useMemo(() => {
    return !!searchQuery || !!statusFilter || !!payrollFilter;
  }, [searchQuery, statusFilter, payrollFilter]);
