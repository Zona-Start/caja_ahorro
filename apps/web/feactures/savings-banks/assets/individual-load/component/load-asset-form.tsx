...
export function LoadAssetsForm({ selectedAssociate, isSubmitting, onSubmit }: LoadAssetsFormProps) {
  ...
  useEffect(() => {
    if (selectedAssociate) {
      form.setValue("associateAccountId", selectedAssociate.associateAccountsId)
    } else {
    ...
    <Card>
      <CardHeader>
        <CardTitle className="flex items-center gap-2">
          <IconWrapper color="green" className="w-8 h-8">
            <PlusCircle />
          </IconWrapper>
          Datos del Depósito
        </CardTitle>
        <CardDescription>Complete la información para registrar el depósito</CardDescription>
      </CardHeader>
      <CardContent>
        <Form {...form}>
                        ...
                        render={({ field }) => (
                          <FormItem className="col-span-2 w-full">
                            ...
                            >
                              ...
                              <SelectContent>
                                {Object.entries(ASSOCIATE_MOVEMENT_TYPES).map(([value, label]) => (
...
<FormField
            ...
            <Button type="submit" className="w-full" disabled={!selectedAssociate || isSubmitting}>
              {isSubmitting ? (
                <>
