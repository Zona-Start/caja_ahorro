...
import { usePaymentsByAccountPayable } from '../hooks/use-query-payment-history';
...
export function AccountPayableDetails({ accountPayable }: DetailsProps) {
  ...
  const { data: paymentHistory, isLoading: isLoadingHistory } =
    usePaymentsByAccountPayable(accountPayable.id);

  const payments = paymentHistory?.data.filter(
    (p) => !['ADVANCE', 'CREDIT_NOTE'].includes(p.paymentType),
  );
  const appliedCredits = paymentHistory?.data.filter((p) =>
    ['ADVANCE', 'CREDIT_NOTE'].includes(p.paymentType),
  );
  ...
  return (
    <ScrollArea className="h-[75vh] p-1">
      <div className="space-y-4 p-4">
                ...
                {payments.map((payment) => (
                  <TableRow key={payment.id}>
                    ...
                    <TableCell className="text-right">
                      {payment.totalAmount.toFixed(2)} Bs.
                    </TableCell>
                  </TableRow>
                ))}
              </TableBody>
        ...
        <div className="p-4 border rounded-lg">
          <h3 className="font-semibold text-lg mb-2">
            Anticipos y Notas de Crédito Aplicadas
          </h3>
          {isLoadingHistory ? (
            <p>Cargando...</p>
              ...
              <TableBody>
                {appliedCredits.map((credit) => (
                  <TableRow key={credit.id}>
                    <TableCell>{credit.paymentNumber}</TableCell>
                    <TableCell>{credit.paymentType}</TableCell>
                    <TableCell className="text-right">
                      {credit.totalAmount.toFixed(2)} Bs.
                    </TableCell>
                  </TableRow>
                ))}
              </TableBody>
