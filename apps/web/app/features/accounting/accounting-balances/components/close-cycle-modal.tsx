...
import { useCloseCycleMutation } from '../hooks/use-accounting-balances-mutation';
...
export function CloseCycleModal({ open, onOpenChange }: CloseCycleModalProps) {
  ...
  const openCycles = cyclesData?.filter((cycle: any) => cycle.status === 'OPEN') || [];

  const selectedCycle = openCycles.find((c: any) => c.id?.toString() === cycleId);
