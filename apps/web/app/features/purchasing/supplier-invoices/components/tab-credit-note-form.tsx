...
import { creditNoteFormSchema, type CreditNoteForm } from '../schemas/supplier-invoice.schema';

interface Props {
  onSuccess?: () => void;
  onCancel?: () => void;
  suppliers: { id: string; name: string }[];
  saveMutation: (data: CreditNoteForm, opts: { onSuccess: () => void }) => void;
  isSaving: boolean;
}

export function TabCreditNoteForm({ onSuccess, onCancel, suppliers, saveMutation, isSaving }: Props) {
      ...
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
        ...
        <FormField control={form.control} name="reason" render={({ field }) => (
          <FormItem>
            ...
            <FormControl><Textarea placeholder="Motivo de la nota de crédito" {...field} rows={2} /></FormControl>
            <FormMessage />
          </FormItem>
        )} />
        ...
        <div className="flex justify-end gap-4 pt-4">
          ...
          <Button type="submit" disabled={isSaving}>{isSaving ? 'Guardando...' : 'Guardar Nota de Crédito'}</Button>
        </div>
      </form>
    </Form>
  );
