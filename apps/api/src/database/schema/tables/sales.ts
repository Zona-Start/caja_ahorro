...
export const salesInvoices = salesSchema.table('sales_invoices', {
  ...
  deliveryNoteId: uuid('delivery_note_id').references(
...
export const customerPaymentApplications = salesSchema.table(
...
export const salesDeliveryNoteItems = salesSchema.table(
  'sales_delivery_note_items',
  {
    id: uuid('id').defaultRandom().primaryKey(),
    deliveryNoteId: uuid('delivery_note_id')
      .notNull()
      .references(() => salesDeliveryNotes.id, { onDelete: 'cascade' }),
    productId: uuid('product_id').notNull(),
    quantity: numeric('quantity', { precision: 12, scale: 4 }).notNull(),
  },
);
