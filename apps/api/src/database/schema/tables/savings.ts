...
export const associates = savingsSchema.table(
  'associates',
  {
    ...
    payrollTypeId: uuid('payroll_type_id').references(() => categories.id, {
      onDelete: 'set null',
    }), // tipo de nomina
    associatedTypeId: uuid('associated_type_id').references(
      () => categories.id,
      {
        onDelete: 'set null',
      },
...
export const associateAccounts = savingsSchema.table(
  'associate_accounts',
  {
    id: uuid('id').primaryKey().defaultRandom(),
    ...
    bankDirectoryId: uuid('bank_id').references(() => bankDirectory.id, {
      onDelete: 'set null',
    }), // id del banco
...
export const associateAccountMovements = savingsSchema.table(
  'associate_account_movements',
  {
    id: uuid('id').primaryKey().defaultRandom(),
    associateAccountId: uuid('associate_account_id')
      .notNull()
...
// historial de saldo
export const associateAccountBalanceHistory = savingsSchema.table(
  'associate_account_balance_history',
  {
    ...
    movementId: uuid('movement_id').references(
...
export const withdrawalTypes = savingsSchema.table(
  'withdrawal_types',
  {
    id: uuid('id').primaryKey().defaultRandom(),
    tenantId: uuid('tenant_id')
    ...
    accountDebit: uuid('account_debit').references(() => accountPlan.id, {
      onDelete: 'set null',
    }), // Cuenta contable para el débito del retiro
    expenseAccount: uuid('expense_account').references(() => accountPlan.id, {
      onDelete: 'set null',
    ...
    withdrawalFrequencyRelation: uuid('category_id').references(
      () => categories.id, // Relación con una tabla de categorías (ej: categorías de asociados para diferentes frecuencias de retiro)
      { onDelete: 'set null' },
    ),
...
//tabla de retiros
export const withdrawalsAssociates = savingsSchema.table(
  'withdrawals_associates',
  {
    ...
    associateAccountId: uuid('associate_account_id')
      .notNull()
      .references(() => associateAccounts.id, { onDelete: 'cascade' }),
    withdrawalTypeId: uuid('withdrawal_type_id').references(
    ...
    commercialHouseId: uuid('commercial_house_id').references(
      () => suppliers.id,
      { onDelete: 'set null' },
    ),
...
// tipos de prestamos
export const loanTypes = savingsSchema.table(
  'loan_types',
  {
    id: uuid('id').primaryKey().defaultRandom(), // ID único del tipo de préstamo
    tenantId: uuid('tenant_id')
      .references(() => tenants.id, {
        onDelete: 'cascade',
      })
    ...
    expenseAccountChartId: uuid('expense_account_chart_id').references(
      () => accountPlan.id,
      { onDelete: 'set null' },
...
export const loans = savingsSchema.table(
  'loans',
  {
    id: uuid('id').primaryKey().defaultRandom(), // ID único del préstamo
    tenantId: uuid('tenant_id')
      .references(() => tenants.id, {
        onDelete: 'cascade',
      })
    ...
    loanTypeId: uuid('loan_type_id') // Tipo de préstamo (FK a tabla de tipos)
      .notNull()
    ...
    previousLoanId: uuid('previous_loan_id').references(() => loans.id), // Relación con préstamo anterior si existe
    paymentMethod: paymentMethodEnum('payment_method'), // Forma de pago (transferencia, cheque, efectivo)
    disbursementAccountId: uuid('disbursement_account_id') // Cuenta del asociado donde se desembolsa
    ...
    approvedByUserId: uuid('approved_by_user_id').references(() => users.id), // Usuario que aprueba
    disbursedByUserId: uuid('disbursed_by_user_id').references(() => users.id), // Usuario que desembolsa
...
export const loanAmortizationSchedule = savingsSchema.table(
...
export const loanStatusHistory = savingsSchema.table(
  'loan_status_history',
  {
    id: uuid('id').primaryKey().defaultRandom(),
    loanId: uuid('loan_id')
    ...
    changedByUserId: uuid('changed_by_user_id').references(() => users.id), // Usuario que realizó el cambio
...
export const loanPayments = savingsSchema.table(
  'loan_payments',
  {
    id: uuid('id').primaryKey().defaultRandom(),
    tenantId: uuid('tenant_id')
...
// tabla registro de los pagos
export const loanPaymentsDetails = savingsSchema.table(
...
// tipos de creditos
export const creditsTypes = savingsSchema.table(
  'credits_types',
  {
    id: uuid('id').primaryKey().defaultRandom(), // ID único del tipo de credito
    tenantId: uuid('tenant_id')
      .references(() => tenants.id, {
        onDelete: 'cascade',
      })
    ...
    creditAccountChartId: uuid('credit_account_chart_id').references(
      () => accountPlan.id,
      { onDelete: 'set null' },
...
export const credits = savingsSchema.table(
  'credits',
  {
    id: uuid('id').primaryKey().defaultRandom(), // ID único del préstamo
    tenantId: uuid('tenant_id')
    ...
    associateId: uuid('associate_id')
      .notNull()
      .references(() => associates.id, { onDelete: 'restrict' }), // FK al asociado
    creditTypeId: uuid('credit_type_id') // Tipo de credito (FK a tabla de tipos)
      .notNull()
    ...
    expensesAmount: numeric('expenses_amount', { precision: 20, scale: 6 }), // Monto de gastos administrativos
    overdraftAmount: numeric('overdraft_amount', { precision: 20, scale: 6 }), // Sobregiro si aplica
    previousCreditId: uuid('previous_credit_id').references(() => credits.id), // Relación con credito anterior si existe
...
// Tabla de amortización del préstamo
export const creditAmortizationSchedule = savingsSchema.table(
...
// Tabla para registrar cambios de estado del préstamo
export const creditStatusHistory = savingsSchema.table(
...
// tabla registro de los pagos
export const creditPayments = savingsSchema.table(
  'credit_payments',
  {
    id: uuid('id').primaryKey().defaultRandom(),
    tenantId: uuid('tenant_id')
...
// tabla registro de los pagos
export const creditPaymentsDetails = savingsSchema.table(
  'credit_payment_details',
  {
    id: uuid('id').primaryKey().defaultRandom(),
    creditPaymentId: uuid('credit_payment_id')
      .notNull()
...
export const liquidationsAssociates = savingsSchema.table(
...
export const creditItemSales = savingsSchema.table(
  'credit_item_sales',
  {
    id: uuid('id').primaryKey().defaultRandom(),
    tenantId: uuid('tenant_id')
    ...
    deliveryStatus: varchar('delivery_status', { length: 50 })
      .notNull()
      .default('ENTREGADO'),
    days: uuid('days').references(() => categories.id, {
      onDelete: 'cascade',
    }),
    ...timestamps,
  },
...
export const paymentBatches = savingsSchema.table('payment_batches', {
  id: uuid('id').primaryKey().defaultRandom(),
  tenantId: uuid('tenant_id')
    .references(() => tenants.id, {
      onDelete: 'cascade',
...
export const paymentBatchItems = savingsSchema.table('payment_batch_items', {
  id: uuid('id').primaryKey().defaultRandom(),
  ...
  sourceId: uuid('source_id').notNull(), // id en loans, withdrawals o liquidations
  associateAccountId: uuid('associate_account_id').references(
