...
export class GlobalInventorySubscriber implements OnModuleInit {
  ...
  onModuleInit() {
    this.eventBus.subscribe(
      PURCHASING_EVENTS.ORDER_CREATED,
      (envelope: EventEnvelope) => {
        this.logger.log(
          `Purchase order created — inventory commitment: ${envelope.payload.orderNumber}`,
        );
      },
    );
    ...
    this.eventBus.subscribe(
      ...
      (envelope: EventEnvelope) => {
        this.logger.log(
          `Invoice accounted — inventory stock update: ${envelope.payload.invoiceNumber}`,
        );
      },
    );
