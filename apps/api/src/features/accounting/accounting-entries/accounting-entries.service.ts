...
//   private async validateAccountingEntry(entry: CreateAccountingEntryDto) {
//     const { companyId, accountingCycleId, entryDate, details } = entry;

//     // 1. Validate Accounting Cycle
//     const accountingCycle =
//       await this.accountingCyclesService.findOne(accountingCycleId);
//     if (!accountingCycle || accountingCycle.status !== 'OPEN') {
//       throw new BadRequestException(
//         'El ciclo contable debe estar abierto para crear o modificar asientos.',
//       );
//     }

//     // 2. Validate Entry Date within Cycle Dates
//     const cycleStartDate = new Date(accountingCycle.startDate);
//     const cycleEndDate = new Date(accountingCycle.endDate);
//     const entryDateObj = new Date(entryDate);

//     if (entryDateObj < cycleStartDate || entryDateObj > cycleEndDate) {
//       throw new BadRequestException(
//         'La fecha del asiento debe estar dentro del rango del ciclo contable abierto.',
//       );
//     }
...
//     let totalDebit = 0;
//     let totalCredit = 0;
//     const accountPlanIds = details.map((d) => d.accountPlanId);
...
//       .where(inArray(schema.accountPlan.id, accountPlanIds));
//     const accountMap = new Map(accounts.map((acc) => [acc.id, acc]));

//     for (const detail of details) {
//       const account = accountMap.get(detail.accountPlanId);
//       if (!account) {
//         throw new BadRequestException(
//           `Cuenta contable con ID ${detail.accountPlanId} no encontrada.`,
//         );
//       }
...
//       if (
//         (detail.debit > 0 && detail.credit > 0) ||
//         (detail.debit === 0 && detail.credit === 0)
//       ) {
//         throw new BadRequestException(
//           'Cada línea de detalle debe tener un débito o un crédito, pero no ambos ni ninguno.',
//         );
//       }
//     }
...
//       throw new BadRequestException(
//         'El asiento no puede tener débitos y créditos en cero.',
//       );
//     }
//   }
...
//     await this.validateAccountingEntry(createAccountingEntryDto);

//     const { details, ...entryData } = createAccountingEntryDto;

//     const [newEntry] = await this.drizzle
//       .insert(schema.accountingEntries)
//       .values({
//         ...entryData,
//         entryDate: entryData.entryDate.toISOString().split('T')[0],
...
//     return {
//       ...newEntry,
//       entryDate: new Date(newEntry.entryDate), // Convert to Date
//       postedAt: newEntry.postedAt ? new Date(newEntry.postedAt) : null, // Convert to Date if exists
//       details: entryDetails as AccountingEntryDetail[],
//     } as AccountingEntry;
//   }

//   async findAllPaginated(
//     filterDto: FilterAccountingEntryDto,
//   ): Promise<{ data: AccountingEntry[]; meta: any }> {
//     const {
//       page = 1,
//       limit = 10,
//       search,
...
//     }
//     if (status) {
//       searchConditions.push(eq(schema.accountingEntries.status, status));
//     }
...
//     if (originType) {
//       searchConditions.push(
//         eq(schema.accountingEntries.originType, originType),
//       );
//     }
...
//     }
//     if (accountPlanId) {
//       // Subquery to check if the accountPlanId exists in accountingEntryDetails for the entry
//       const entriesWithAccount = this.drizzle
...
//       searchConditions.push(
//         inArray(schema.accountingEntries.id, entriesWithAccount),
//       );
//     }
...
//       .orderBy(
//         sortOrder === 'asc'
//           ? sql`${schema.accountingEntries[sortBy]} asc`
//           : sql`${schema.accountingEntries[sortBy]} desc`,
//       )
//       .limit(limit)
//       .offset(offset);

//     // Transform the data to the correct type
//     const data: AccountingEntry[] = entries.map((entry) => ({
//       ...entry,
...
//       // You may also need to handle other date fields like `createdAt` and `updatedAt`
//     }));
...
//     userId: number,
//     id: number,
//     updateAccountingEntryDto: UpdateAccountingEntryDto,
//   ): Promise<AccountingEntry> {
//     const existingEntry = await this.findOne(id);

//     if (existingEntry.status !== entryStatusEnum.DRAFT) {
//       throw new BadRequestException(
//         'Solo se pueden editar asientos en estado BORRADOR.',
//       );
//     }

//     // Merge existing entry with update DTO for validation
//     const mergedEntry = {
//       ...existingEntry,
//       ...updateAccountingEntryDto,
//       entryDate: updateAccountingEntryDto.entryDate || existingEntry.entryDate,
//       accountingCycleId:
//         updateAccountingEntryDto.accountingCycleId ||
//         existingEntry.accountingCycleId,
//       details:
//         updateAccountingEntryDto.details &&
//         updateAccountingEntryDto.details.length > 0
//           ? updateAccountingEntryDto.details.map((detail) => ({
//               ...existingEntry?.details?.find((ed) => ed.id === detail.id),
//               ...detail,
//               debit:
...
//     await this.validateAccountingEntry(mergedEntry);

//     const { details, ...entryData } = updateAccountingEntryDto;

//     const [updatedEntry] = await this.drizzle
//       .update(schema.accountingEntries)
//       .set({
//         ...entryData,
//         entryDate: entryData.entryDate
//           ? entryData.entryDate.toISOString().split('T')[0]
...
//         // Asegura que accountPlanId siempre sea un número
//         const accountPlanId =
//           detail.accountPlanId || existingDetail?.accountPlanId;
//         if (accountPlanId === undefined) {
//           throw new BadRequestException(
//             'El plan de cuenta es requerido para cada detalle de asiento.',
//           );
//         }
...
//           // Asigna el valor ya validado
//           accountPlanId: accountPlanId,
//           debit: (detail.debit !== undefined
//             ? detail.debit
//             : existingDetail
//               ? Number(existingDetail.debit)
//               : 0
//           ).toString(),
//           credit: (detail.credit !== undefined
//             ? detail.credit
//             : existingDetail
//               ? Number(existingDetail.credit)
//               : 0
//           ).toString(),
//         };
//       });
...
//     if (existingEntry.status !== entryStatusEnum.DRAFT) {
//       throw new BadRequestException(
//         'Solo se pueden eliminar asientos en estado BORRADOR.',
//       );
//     }

//     // Check for dependencies (e.g., if this entry was part of a reversal, though not explicitly in schema)
//     // For now, assuming no external dependencies prevent deletion if DRAFT.
...
//     if (existingEntry.status !== entryStatusEnum.DRAFT) {
//       throw new BadRequestException(
//         'Solo se pueden someter asientos en estado BORRADOR.',
//       );
//     }

//     if (!existingEntry.details || existingEntry.details.length === 0) {
//       throw new BadRequestException(
//         'El asiento contable debe tener al menos dos líneas de detalle para ser sometido.',
//       );
//     }

//     // Transform existingEntry to CreateAccountingEntryDto for validation
//     const entryToValidate: CreateAccountingEntryDto = {
//       companyId: existingEntry.companyId,
//       accountingCycleId: existingEntry.accountingCycleId,
//       entryDate: existingEntry.entryDate,
//       description: existingEntry.description,
//       originReferenceId: existingEntry.originReferenceId || undefined,
//       originType: existingEntry.originType || undefined,
//       currencyCode: existingEntry.currencyCode,
//       details: existingEntry.details.map((detail) => ({
...
//     await this.validateAccountingEntry(entryToValidate);
...
//     if (existingEntry.status !== entryStatusEnum.PENDING) {
//       throw new BadRequestException(
//         'Solo se pueden contabilizar asientos en estado PENDIENTE.',
//       );
//     }

//     // Generate unique voucher number (example: simple increment or UUID)
//     // For now, let's just set postedAt
//     const [updatedEntry] = await this.drizzle
//       .update(schema.accountingEntries)
//       .set({
...
//     // TODO: Update cached balances if applicable (accountingBalance view)
...
//     if (existingEntry.status !== entryStatusEnum.POSTED) {
//       throw new BadRequestException(
//         'Solo se pueden anular asientos en estado CONTABILIZADO.',
//       );
//     }

//     if (!existingEntry.id) {
//       throw new BadRequestException(
//         'El asiento a anular no tiene un ID válido.',
//       );
//     }

//     if (!existingEntry.details || existingEntry.details.length === 0) {
//       throw new BadRequestException(
//         'El asiento contable no tiene detalles para ser anulado.',
//       );
//     }

//     // Create a reversal entry
//     const reversalEntryData: CreateAccountingEntryDto = {
//       companyId: existingEntry.companyId,
//       accountingCycleId: existingEntry.accountingCycleId,
//       entryDate: new Date(), // Reversal date is today
//       description: `ANULACIÓN: ${existingEntry.description} (Original ID: ${existingEntry.id})`,
//       originReferenceId: existingEntry.id.toString(),
//       originType: 'REVERSAL',
//       currencyCode: existingEntry.currencyCode,
//       details: existingEntry.details.map((detail) => ({
//         accountPlanId: detail.accountPlanId,
//         debit: Number(detail.credit), // Swap debit and credit, convert to number
//         credit: Number(detail.debit), // Swap debit and credit, convert to number
//         description: `Reversión de ${detail.description || ''}`,
//       })),
//     };

//     const reversalEntry = await this.create(userId, reversalEntryData);

//     // Update original entry status to CANCELLED
//     const [cancelledEntry] = await this.drizzle
//       .update(schema.accountingEntries)
//       .set({
//         status: entryStatusEnum.CANCELLED,
//         updatedById: userId,
//       })
//       .where(eq(schema.accountingEntries.id, id))
//       .returning();
