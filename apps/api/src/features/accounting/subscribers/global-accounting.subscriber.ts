  ...
  onModuleInit() {
    this.eventBus.subscribe(
      ...
      (envelope: EventEnvelope) => {
        this.logger.log(
          `Loan payment created — accounting flow triggered: ${envelope.payload.customReference}`,
        );
      },
    );
    ...
    this.eventBus.subscribe(
      ...
      (envelope: EventEnvelope) => {
        this.logger.log(
          `Inventory movement — accounting flow triggered: ${envelope.payload.movementId}`,
        );
      },
    );

    this.eventBus.subscribe(
      BANKING_EVENTS.MOVEMENT_CREATED,
      (envelope: EventEnvelope) => {
        this.logger.log(
          `Bank movement created — accounting flow triggered: ${envelope.payload.movementId}`,
        );
      },
    );

    this.eventBus.subscribe(
      PURCHASING_EVENTS.SUPPLIER_INVOICE_ACCOUNTED,
      (envelope: EventEnvelope) => {
        this.logger.log(
          `Supplier invoice accounted — accounting sync: ${envelope.payload.invoiceNumber}`,
        );
      },
    );
