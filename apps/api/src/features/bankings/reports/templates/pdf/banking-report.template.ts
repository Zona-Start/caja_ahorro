...
export function buildReconciliationActTable(data: any): Content {
  ...
  return [
    {
      text: `Cuenta: ${data.bankAccount?.accountName || ''} - ${data.bankAccount?.accountNumber || ''}`,
      margin: [0, 0, 0, 4],
    },
    {
      text: `Fecha Corte: ${fmtDate(r?.statementDate)} | Estado: ${r?.status}`,
      margin: [0, 0, 0, 10],
    },
      ...
      table: {
        ...
        body: [
          [
            ...
            Number(r?.statementEndingBalance || 0).toFixed(2),
          ],
          [
            'Saldo según Libros (Antes)',
            Number(r?.bookBalanceBefore || 0).toFixed(2),
          ],
          [
            'Saldo según Libros (Después)',
            Number(r?.bookBalanceAfter || 0).toFixed(2),
          ],
    ...
    {
      text: `Líneas del Extracto (${lines.length})`,
      bold: true,
      margin: [0, 8, 0, 4],
    },
    {
    ...
    {
      text: `Transacciones Conciliadas (${txns.length})`,
      bold: true,
      margin: [0, 8, 0, 4],
    },
    {
...
export function buildPendingItemsTable(data: any): Content {
  ...
  return [
    {
      text: `Partidas con más de ${data.daysThreshold} días sin conciliar (${data.count})`,
      margin: [0, 0, 0, 10],
    },
    {
      layout: 'lightHorizontalLines',
          ...
          ...items.map((t: any) => {
            ...
            return [
              ...
              Number(t.creditAmount) > 0
                ? Number(t.creditAmount).toFixed(2)
                : '',
              String(age),
            ];
          }),
