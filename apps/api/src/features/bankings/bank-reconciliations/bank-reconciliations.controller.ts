...
export class BankReconciliationsController {
  ...
  @ApiOperation({
    summary: 'Upload Excel and create reconciliation with statement lines',
  })
  async uploadExcel(
    @Req() req: any,
  ...
  @ApiOperation({
    summary: 'Add statement line (stores in bank_statement_lines only)',
  })
  async addStatementLine(
    @Param('id') id: string,
    @Body(new ZodValidatorPipe(AddStatementLineSchema)) dto: any,
    @Req() req: any,
  ) {
  ...
  @ApiOperation({
    summary: 'Get bank transactions (book entries) available for matching',
  })
  ...
  @Post(':id/auto-match')
  @ApiOperation({
    summary: 'Auto-match statement lines against book transactions',
  })
  ...
  @ApiOperation({
    summary: 'Manual match: 1:1, 1:N, N:1 with amount validation',
  })
  async manualMatch(
    @Param('id') id: string,
    @Body() dto: any,
  ...
  @ApiOperation({
    summary: 'Create bank transaction from statement line and auto-match',
  })
  async generateBookEntry(
    @Param('id') id: string,
    @Body(new ZodValidatorPipe(GenerateBookEntrySchema)) dto: any,
    @Req() req: any,
  ) {
    const { targetTenantId, userId } =
      this.tenantContextService.getTenantContext(req, dto);
  ...
  @ApiOperation({
    summary:
      'Get all bank reconciliations, optionally filtered by bankAccountId',
  })
  async findAll(
    @Req() req: any,
  ...
  @Get(':id')
  @ApiOperation({
    summary: 'Get reconciliation by ID with statement lines and details',
  })
