    ...
    try {
      return await this.drizzle.transaction(async (tx) => {
        ...
        const existingMovement =
          await  tx.select().from(associateAccountMovements).where(and(
              eq(
                associateAccountMovements.associateAccountId,
                associateAccountId,
              ),
              eq(associateAccountMovements.movementType, movementType),
        ...
        const account = await tx.select().from(associateAccounts).where(eq(associateAccounts.id, associateAccountId));
        ...
        const reference = referenceNumber ?? generateUniqueReference('HABERES')

        const exchangeRate = currencyCode !== 'VES' ? await this.getExchangeRate(currencyCode, transactionDate ?? new Date()) : null;
        ...
        const [newMovement] = await tx
          ...
          .returning({ id: associateAccountMovements.id, referenceNumber: associateAccountMovements.referenceNumber });
