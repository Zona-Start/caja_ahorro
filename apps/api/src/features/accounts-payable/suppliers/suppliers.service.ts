import { suppliers } from '@/database/schema/accounts-payable';
...
export class SuppliersService {
  ...
  async findAll(
    ...
    const data = await this.drizzle
      ...
      .orderBy(orderBy)
      .limit(limit);
    // const data = await this.drizzle.query.suppliers.findMany({
    //   where: finalCondition,
    //   limit: limit,
    //   offset: offset,
    //   orderBy: orderBy,
  ...
  async remove(id: number): Promise<{ message: string }> {
    ...
    const invoice = await this.drizzle
      ...
      .where(eq(schema.accountsPayable.supplierId, id));
