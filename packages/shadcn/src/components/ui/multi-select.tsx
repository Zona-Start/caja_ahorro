...
interface MultiSelectProps extends React.ComponentPropsWithoutRef<typeof CommandPrimitive.Input> {
  options: {
    label: string;
    value: string;
  }[];
...
const MultiSelect = React.forwardRef<
  ...
  MultiSelectProps
>(({ options, onChange, value, defaultValue, placeholder, variant, animation, maxSelected, hidePlaceholderWhenSelected, loop, disabled, className, ...props }, ref) => {
            ...
            <div className="flex w-full items-center justify-between">
              <div className="flex flex-wrap items-center gap-2">
                ...
                {value.length > maxSelected && (
                  <Badge variant="secondary">+{value.length - maxSelected}</Badge>
                )}
              </div>
